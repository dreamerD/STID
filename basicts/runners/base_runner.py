...
class BaseRunner(Runner):
    ...
    def build_model(self, cfg: dict) -> nn.Module:
        ...
        if torch.distributed.is_initialized():
            model = DDP(model, device_ids=[get_local_rank()], find_unused_parameters=cfg.get(
    ...
    def train(self, cfg: dict):
        ...
        for epoch_index in range(self.start_epoch, self.num_epochs):
            ...
            # tqdm process bar
            data_iter = tqdm(self.train_data_loader) if get_local_rank(
            ) == 0 else self.train_data_loader
            ...
            expected_end_time = train_time_predictor.get_expected_end_time(
                epoch)
    ...
    def validate_data_loop(self, train_epoch: int = None):
        ...
        for iter_index, data in enumerate(self.val_data_loader):
            self.val_iters(data, train_epoch=train_epoch,
